...
public interface RecipeRepository extends JpaRepository<Recipe, Long> {
    ...
    @Query(value="SELECT DISTINCT r.* FROM recipe r JOIN recipe_category_map m ON r.id = m.recipe_id WHERE m.category IN (:categories) AND r.id != :recipeId ORDER BY RANDOM() LIMIT 6", nativeQuery=true)
